    ...
    <div className="flex-1 p-6 md:p-10 animate-in fade-in zoom-in-95 duration-500 max-w-[1400px] mx-auto min-h-[calc(100vh-4rem)] space-y-10">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
        ...
        <div className="w-full lg:w-96 relative group">
          <Input 
            ...
            className="h-12 rounded-2xl bg-[#0f111a] border-white/5 text-white pl-12 pr-4 font-bold focus:ring-indigo-500"
          />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {filteredCourses.map(course => (
          <Link to={`/dashboard/courses/${course.id || course._id}`} key={course.id || course._id} className="group rounded-3xl bg-indigo-950/10 backdrop-blur-md border border-white/10 hover:border-indigo-500/30 shadow-xl transition-all p-8 flex flex-col justify-between relative overflow-hidden block">
            ...
            <div className="relative z-10 space-y-4">
              ...
              <h3 className="text-xl font-bold text-slate-200">{course.name}</h3>
        ...
        {filteredCourses.length === 0 && (
          <div className="col-span-full p-12 text-center rounded-3xl border border-white/10 border-dashed flex flex-col items-center gap-4">
            <BookOpen className="h-12 w-12 text-slate-500/50" />
            <p className="text-sm font-medium text-slate-400">No modules found for your account.</p>
          </div>
        )}
      </div>
    </div>
