...
import {
    ...
    Users, Search, X, BookOpen, Building2,
...
interface TaskModalProps {
    ...
    onClose: () => void;
    task?: Task | null;
    onSave: (payload: CreateTaskPayload) => Promise<Task | void>;
...
export function TaskModal({ open, onClose, task, onSave, contextDepartmentId, contextCourseId }: TaskModalProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (targetType === 'COURSE') payload.target.course_id = contextCourseId;
            else if (targetType === 'DEPARTMENT') payload.target.department_id = contextDepartmentId || deptId;
            else if (targetType === 'SPECIFIC') payload.target.specific_users = selectedUserIds;
