...
import { Users, FileText, CheckCircle, TrendingUp, AlertTriangle, UserCheck, BrainCircuit, BarChart3 } from 'lucide-react';
...
import type { CourseAnalyticsResult } from '@/shared/api/courseApi';
...
export function CourseAnalyticsDashboard({ courseId }: { courseId: string }) {
  ...
  const [riskThreshold, setRiskThreshold] = useState<number>(50); // Default risk threshold is 50%

  useEffect(() => {
    if (!courseId || viewMode !== 'stats') return;
    const fetchAnalytics = async () => {
      try {
        setIsLoading(true);
        const result = await courseApi.getCourseAnalytics(courseId);
        setData(result);
      } catch (error) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="p-6 rounded-3xl bg-panel border border-panel shadow-2xl flex flex-col justify-between group hover:border-indigo-500/50 transition-all">
          ...
          <div className="mt-4">
            ...
            <p className="text-[11px] font-bold text-content-muted uppercase tracking-widest mt-1">Enrolled Students</p>
          </div>
        </div>
        ...
        <div className="p-6 rounded-3xl bg-panel border border-panel shadow-2xl flex flex-col justify-between group hover:border-purple-500/50 transition-all">
          ...
          <div className="mt-4">
            ...
            <p className="text-[11px] font-bold text-content-muted uppercase tracking-widest mt-1">Total Tasks Assigned</p>
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-content-muted mt-1 font-medium">
              Students falling below this completion rate or average grade will be flagged as "At Risk".
            </p>
          </div>
      ...
      <div className="rounded-3xl bg-panel border border-panel shadow-2xl overflow-hidden">
        <div className="p-6 border-b border-panel flex justify-between items-center">
          <div>
            ...
            <p className="text-sm font-medium text-content-muted mt-1">Detailed breakdown of student engagement and grades</p>
          </div>
          <div className="px-4 py-2 bg-indigo-500/10 text-indigo-500 font-bold rounded-xl text-sm">
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-panel">
              {data.students.map((student) => {
                const studentCompletionRate = data.kpis.totalTasks > 0 
                  ? (student.tasksCompleted / data.kpis.totalTasks) * 100 
                  : 0;
                
                const isAtRisk = studentCompletionRate < riskThreshold || student.averageGrade < riskThreshold;
                  ...
                  <tr key={student.userId} className="hover:bg-panel-hover/50 transition-colors">
                    ...
                    <td className="px-6 py-5">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-content-muted text-xs">/ {data.kpis.totalTasks}</span>
                      </div>
                    </td>
                    <td className="px-6 py-5">
                      <div className="flex items-center gap-2">
